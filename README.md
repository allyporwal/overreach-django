...
(**) Although the views protect against any tinkering in Developer Tools by a user, some Javascript was also necessary as an additional step. A server 500 error can be triggered by submitting the workout form with just an exercise, reps and set count but nothing else. This is why the save button is disabled until there is some weights, reps and sets input. Removing the required attribute on any weight, reps or RPE inputs on the form and inputting it blank is caught by the views and the entire workout is discarded. There is a validators.py file that prevents other errors caused by tinkering in Developer tools and "rescues" the workout and presents a list of erros, for example if a user changes the minimum input allowed on a reps input field and inputs -1 reps, the validator will catch that. In normal use on a mobile device this shouldn't be a problem but it is worth noting. Every effort has been made to ensure only valid data is input to the workouts table.
